<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Yönetici Paneli - Randevular') }}
        </h2>
    </x-slot>

    <x-slot name="head">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" integrity="sha512-ieoQ3yO8W+f1I1r1tF7mR4gX3GgqVv2C2qL2l6e7Q5V+fP6f0F8jQ4R2Gg2L6A==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    </x-slot>
    ...
    <div class="py-12 bg-gray-100" x-data="{ activeTab: (new URLSearchParams(window.location.search)).get('tab') || 'upcoming', ...appointmentList() }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8">
                <h3 class="text-2xl font-bold text-gray-900 mb-6">Tüm Randevular</h3>
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        @forelse ($upcomingAppointments as $appointment)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="py-4 px-6">{{ $appointment->doctor ? $appointment->doctor->name : 'Doktor Silinmiş' }}</td>
                                ...
                                <td class="py-4 px-6 flex space-x-2">
                                    @if(!$appointment->is_approved)
                                        ...
                                        <form id="reject-form-{{ $appointment->id }}" action="{{ route('admin.appointments.reject', $appointment) }}" method="POST" @submit.prevent>
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr class="bg-white">
                                <td colspan="7" class="py-4 px-6 text-center text-gray-600">Henüz onay bekleyen veya gelecek bir randevu bulunmamaktadır.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
                ...
                <div x-show="activeTab === 'past'" style="display: none;" class="overflow-x-auto relative shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        @forelse ($pastAppointments as $appointment)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="py-4 px-6 flex items-center justify-center">
                                    @if($appointment->is_completed)
                                        <form action="{{ route('admin.appointments.incomplete', $appointment) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                        ...
                        @empty
                            <tr class="bg-white">
                                <td colspan="7" class="py-4 px-6 text-center text-gray-600">Henüz geçmiş randevu bulunmamaktadır.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
